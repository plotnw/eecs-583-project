...
bool LoopRollerCFG::run() {
  ...
  std::vector<BasicBlock *> allowed_dom_targets;
  allowed_dom_targets.push_back(curr_exit_block);
  for (BasicBlock *bb : dominators_of_terminator) {
    allowed_dom_targets.push_back(bb);  
  }
  bool left_first_not_in = (std::find(allowed_dom_targets.begin(), allowed_dom_targets.end(), left_children_of_ifs.at(0)) == allowed_dom_targets.end());
    
  for (BasicBlock *bb : left_children_of_ifs) {
    bool bb_not_in = std::find(allowed_dom_targets.begin(), allowed_dom_targets.end(), left_children_of_ifs.at(0)) == allowed_dom_targets.end();
    if (bb_not_in != left_first_not_in) {
      errs() << "left children bb mismatch " << bb->getName() << " " << bb_not_in << "," << left_first_not_in << " " << left_children_of_ifs.at(0)->getName();
      return false;
    }
  }

  bool right_first_not_in = (std::find(allowed_dom_targets.begin(), allowed_dom_targets.end(), right_children_of_ifs.at(0)) == allowed_dom_targets.end());
    
  for (BasicBlock *bb : right_children_of_ifs) {
    bool bb_not_in = std::find(allowed_dom_targets.begin(), allowed_dom_targets.end(), right_children_of_ifs.at(0)) == allowed_dom_targets.end();
    if (bb_not_in != right_first_not_in) {
      errs() << "right children bb mismatch " << bb->getName() << " " << bb_not_in << "," << right_first_not_in << " " << right_children_of_ifs.at(0)->getName();
      return false;
    }
  }
  
  errs() << "CFG structure checks passed, attempt to reroll\n";
  ...
  */
  ...
  for (Node *n : G->Nodes) {
    for (Value *v : n->Values) {
      // Some values can appear in multiple places in the AlignmentGraph
      // We want to grab the first reference, since neutral elements we want to grab
      // the inst generated by the BinOpSequence, not the original instruction
      auto it = source_values_to_target_nodes.find(v);
      if (it == source_values_to_target_nodes.end()) {
  ...
  */
  ...
  for (auto mapping_pair : original_instruction_to_rolled) {
    original_instruction_to_rolled_vh[mapping_pair.first] = 
      WeakTrackingVH(mapping_pair.second);;
  }

  for (Instruction &inst : *merged_non_dominators) {
    llvm::RemapInstruction(&inst, original_instruction_to_rolled_vh, 
                           RF_NoModuleLevelChanges | RF_IgnoreMissingLocals);
  }

  merged_non_dominators->dump();

  errs() << "Rerolling left children of ifs\n";

  this->collectSeedInstructions(*merged_non_dominators);

  errs() << "Collected seeds" << "\n";
  for (auto i : Seeds.Stores) {
    errs() << "Store group:\n";
      for (Instruction *inst : i.second) {
        errs() << "\t" << *inst << "\n";
    }
  }
  for (auto i : Seeds.Calls) {
    errs() << "Call group:\n";
      for (Instruction *inst : i.second) {
        errs() << "\t" << *inst << "\n";
    }
