...
static cl::opt<bool>
AlwaysRoll("loop-rolling-cfg-always", cl::init(false), cl::Hidden,
                 cl::desc("Always roll loops, skipping the profitability analysis"));

static cl::opt<int>
SizeThreshold("loop-rolling-cfg-size-threshold", cl::init(2), cl::Hidden,
                 cl::desc("Size threshold for the loop rolling profitability analysis"));

static cl::opt<bool>
MatchAlignment("loop-rolling-cfg-match-alignment", cl::init(false), cl::Hidden,
...
public:
  ...
  AlignedGraphCFG(Node *N, BasicBlock &BB, ScalarEvolution *SE=nullptr) : BB(&BB), SE(SE) {
    Root = N;
    if (Root) {
      addNode(Root);
  ...
  AlignedGraphCFG(BinaryOperator *BO, Instruction *U, BasicBlock &BB, ScalarEvolution *SE=nullptr) : BB(&BB), SE(SE) {
  ...
  template<typename ValueT>
  AlignedGraphCFG(std::vector<ValueT*> &Vs, BasicBlock &BB, ScalarEvolution *SE=nullptr) : BB(&BB), SE(SE) {
...
public:
  CodeGeneratorCFG(Function &F, BasicBlock &BB, AlignedGraphCFG &G) : F(F), BB(BB), G(G) {}
...
class LoopRollerCFG {
public:
  LoopRollerCFG(Function &F, ScalarEvolution *SE) : F(F), SE(SE), NumAttempts(0), NumRolledLoops(0) {}
...
*/
template<typename ValueT>
Node *AlignedGraphCFG::buildReduction(ValueT *V, Instruction *U, BasicBlock &BB, Node *Parent) {
...
*/
template<typename ValueT>
Node *AlignedGraphCFG::buildGEPSequence(std::vector<ValueT *> &VL, BasicBlock &BB, Node *Parent) {
...
template<typename ValueT>
Node *AlignedGraphCFG::buildGEPSequence2(std::vector<ValueT *> &VL, BasicBlock &BB, Node *Parent) {
  errs() << "GEPSeq2\n";
  if (!isa<PointerType>(VL[0]->getType())) return nullptr;
  //auto *Ptr = getUnderlyingObject(VL[0]);
  Type *Ty = nullptr;
...
*/
template<typename ValueT>
Node *AlignedGraphCFG::buildAlternatingSequenceNode(std::vector<ValueT *> &VL, BasicBlock &BB, Node *Parent) {
...
*/
template<typename ValueT>
Node *AlignedGraphCFG::buildBinOpSequenceNode(std::vector<ValueT *> &VL, BasicBlock &BB, Node *Parent) {
  errs() << "BinOP?\n";
  VL[0]->dump();
...
template<typename ValueT>
Node *AlignedGraphCFG::buildRecurrenceNode(std::vector<ValueT *> &Vs, BasicBlock &BB, Node *Parent) {
...
*/
template<typename ValueT>
Node *AlignedGraphCFG::buildConstExprNode(std::vector<ValueT *> &Vs, BasicBlock &BB, Node *Parent) {
...
template<typename ValueT>
Node *AlignedGraphCFG::createNode(std::vector<ValueT*> Vs, BasicBlock &BB, Node *Parent) {
...
void AlignedGraphCFG::growGraph(Node *N, BasicBlock &BB, std::set<Node*> &Visited) {
...
bool AlignedGraphCFG::invalidDependence(Value *V, std::unordered_set<Value*> &Visited) {
...
void CodeGeneratorCFG::generateExtract(Node *N, Instruction * NewI, IRBuilder<> &Builder) {
...
Value *CodeGeneratorCFG::generateMismatchingCode(std::vector<Value *> &VL, IRBuilder<> &Builder) {
...
bool LoopRollingCFG::runImpl(Function &F, ScalarEvolution *SE) {
  LoopRollerCFG RL(F, SE);
  return RL.run();
}

PreservedAnalyses LoopRollingCFG::run(Function &F, FunctionAnalysisManager &AM) {
...
INITIALIZE_PASS(LoopRollingLegacyCFGPass, "loop-rolling-cfg", "Loop rolling over some non-straight-line code", false, false)
